...
public final class FlTerraForgedBiomeSource extends BiomeSource {
    ...
    private static final int STRUCTURE_SAMPLE_CELL_SIZE = 256;
    ...
    private final ThreadLocal<ExactBiomeScope> exactBiomePopulation = new ThreadLocal<>();
     ...
     *
     * <p>The initial BIOMES status only needs broad climate and land semantics. R61 routes these
     * calls through the cheap placement sampler so structure/biome discovery does not cold-start
     * erosion, river maps or complete subsurface snapshots before visible world progress begins.
     * The later NOISE stage replaces these provisional values from the exact immutable snapshot.</p>
     *
    ...
    /** Ends one asynchronous provisional biome-population scope. */
    ...
    /**
     * Activates exact snapshot-backed biome resolution on the current generation thread.
     *
     * <p>The snapshot was already generated by the NOISE owner. Biome refinement therefore reads
     * the same canonical terrain, hydrology and climate objects without another Engine pipeline
     * pass. Blender requests that fall outside the target chunk intentionally use the cheap placement
     * sampler. They must not recursively cold-load an adjacent exact terrain tile merely to blend a
     * quart-biome boundary; that adjacent chunk receives its own exact pass when Minecraft schedules
     * it.</p>
     *
     * @param chunkX chunk X coordinate
     * @param chunkZ chunk Z coordinate
     * @param snapshot exact immutable Engine snapshot for that chunk
     */
    void beginExactBiomePopulation(int chunkX, int chunkZ, ChunkSnapshot snapshot) {
        Objects.requireNonNull(snapshot, "snapshot");
        if (snapshot.chunkX() != chunkX || snapshot.chunkZ() != chunkZ) {
            throw new IllegalArgumentException("Exact biome snapshot does not match the target chunk");
        }
        if (exactBiomePopulation.get() != null) {
            throw new IllegalStateException("Nested exact biome population is not supported");
        }
        exactBiomePopulation.set(new ExactBiomeScope(chunkX, chunkZ, snapshot));
    }

    /** Ends the current-thread exact snapshot-backed biome refinement scope. */
    void endExactBiomePopulation() {
        exactBiomePopulation.remove();
    }
    ...
    public RegistryEntry<Biome> getBiome(
        ...
        ExactBiomeScope exact = exactBiomePopulation.get();
        if (exact != null) {
            TerrainSample sample = exact.sample(current.world, blockX, blockZ);
    ...
    private record ExactBiomeScope(int chunkX, int chunkZ, ChunkSnapshot snapshot) {

        TerrainSample sample(TerrainWorld world, int blockX, int blockZ) {
            int requestedChunkX = Math.floorDiv(blockX, 16);
            int requestedChunkZ = Math.floorDiv(blockZ, 16);
            if (requestedChunkX != chunkX || requestedChunkZ != chunkZ) {
                return world.placementSample(blockX, blockZ);
            }
            int localX = Math.floorMod(blockX, 16);
            int localZ = Math.floorMod(blockZ, 16);
            return snapshot.column(localX, localZ).terrain();
        }
    }
}
